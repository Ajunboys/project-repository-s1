 ...
 *
 * @constructor
 * @param {VvWork.OnScreenKeyboard.Layout} layout
 *     The layout of the on-screen keyboard to display.
 */
VvWork.OnScreenKeyboard = function(layout) {

    /**
     * Reference to this VvWork.OnScreenKeyboard.
     *
     * @private
     * @type {VvWork.OnScreenKeyboard}
     */
    var osk = this;
     ...
     *
     * @returns {VvWork.OnScreenKeyboard.Key}
    ...
    /**
     * Fired whenever the user presses a key on this VvWork.OnScreenKeyboard.
     * 
     * @event
    ...
    /**
     * Fired whenever the user releases a key on this VvWork.OnScreenKeyboard.
     * 
     * @event
     ...
     *
     * @type {VvWork.OnScreenKeyboard.Layout}
     */
    this.layout = new VvWork.OnScreenKeyboard.Layout(layout);
    ...
    /**
     * Resizes all elements within this VvWork.OnScreenKeyboard such that
     * the width is close to but does not exceed the specified width. The
     * height of the keyboard is determined based on the width.
     * 
     * @param {Number} width The width to resize this VvWork.OnScreenKeyboard
     *                       to, in pixels.
     */
     ...
     *
     * @param {Number|String|VvWork.OnScreenKeyboard.Key|VvWork.OnScreenKeyboard.Key[]} object
    ...
    var asKeyArray = function asKeyArray(name, object) {
        ...
        if (object instanceof Array) {
            ...
            for (var i=0; i < object.length; i++) {
                keys.push(new VvWork.OnScreenKeyboard.Key(object[i], name));
            }
            return keys;
        }
        ...
        if (typeof object === 'number') {
            return [new VvWork.OnScreenKeyboard.Key({
                name   : name,
                keysym : object
            })];
        }
        ...
        return [new VvWork.OnScreenKeyboard.Key(object, name)];

    };

    /**
     * Converts the rather forgiving key mapping allowed by
     * VvWork.OnScreenKeyboard.Layout into a rigorous mapping of key name
     * to key definition, where the key definition is always an array of Key
     * objects.
     *
     * @private
     * @param {Object.<String, Number|String|VvWork.OnScreenKeyboard.Key|VvWork.OnScreenKeyboard.Key[]>} keys
     ...
     *
     * @returns {Object.<String, VvWork.OnScreenKeyboard.Key[]>}
     ...
     *
     * @type {Object.<String, VvWork.OnScreenKeyboard.Key[]>}
     */
...
VvWork.OnScreenKeyboard.Layout = function(template) {
     ...
     *
     * @type {Object.<String, Number|String|VvWork.OnScreenKeyboard.Key|VvWork.OnScreenKeyboard.Key[]>}
     */
     ...
     * derived later via a call to resize() on the VvWork.OnScreenKeyboard.
     *
     * @type {Number}
     */
 ...
 * VvWork.OnScreenKeyboard.Key, whether that key is explicitly defined or
 * implied, and may have multiple VvWork.OnScreenKeyboard.Key if behavior
 * depends on modifier states.
 *
 * @constructor
 * @param {VvWork.OnScreenKeyboard.Key|Object} template
 *     The object whose identically-named properties will be used to initialize
 ...
 */
VvWork.OnScreenKeyboard.Key = function(template, name) {
