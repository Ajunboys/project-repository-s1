 ...
 * handling a provided VvWork.InputStream. Data received along the provided
 * stream is to be played back immediately.
 *
 * @constructor
 */
VvWork.AudioPlayer = function AudioPlayer() {

    /**
     * Notifies this VvWork.AudioPlayer that all audio up to the current
 ...
 * implementation of VvWork.AudioPlayer, and thus will be properly handled
 * by VvWork.AudioPlayer.getInstance().
 *
 ...
 *     VvWork.AudioPlayer, false otherwise.
 */
VvWork.AudioPlayer.isSupportedType = function isSupportedType(mimetype) {

    return VvWork.RawAudioPlayer.isSupportedType(mimetype);

};

/**
 * Returns a list of all mimetypes supported by any built-in
 * VvWork.AudioPlayer, in rough order of priority. Beware that only the core
 ...
 *
 * @returns {String[]}
 *     A list of all mimetypes supported by any built-in VvWork.AudioPlayer,
 *     excluding any parameters.
 */
VvWork.AudioPlayer.getSupportedTypes = function getSupportedTypes() {

    return VvWork.RawAudioPlayer.getSupportedTypes();

};

/**
 * Returns an instance of VvWork.AudioPlayer providing support for the given
 * audio format. If support for the given audio format is not available, null
 * is returned.
 *
 * @param {VvWork.InputStream} stream
 *     The VvWork.InputStream to read audio data from.
 *
 * @param {String} mimetype
 *     The mimetype of the audio data in the provided stream.
 *
 * @return {VvWork.AudioPlayer}
 *     A VvWork.AudioPlayer instance supporting the given mimetype and
 *     reading from the given stream, or null if support for the given mimetype
 *     is absent.
 */
VvWork.AudioPlayer.getInstance = function getInstance(stream, mimetype) {

    // Use raw audio player if possible
    if (VvWork.RawAudioPlayer.isSupportedType(mimetype))
        return new VvWork.RawAudioPlayer(stream, mimetype);
...
/**
 * Implementation of VvWork.AudioPlayer providing support for raw PCM format
 * audio. This player relies only on the Web Audio API and does not require any
 * browser-level support for its audio formats.
 *
 * @constructor
 * @augments VvWork.AudioPlayer
 * @param {VvWork.InputStream} stream
 *     The VvWork.InputStream to read audio data from.
 *
 * @param {String} mimetype
 ...
 */
VvWork.RawAudioPlayer = function RawAudioPlayer(stream, mimetype) {

    /**
     * The format of audio this player will decode.
     *
     * @private
     * @type {VvWork.RawAudioPlayer._Format}
     */
    var format = VvWork.RawAudioPlayer._Format.parse(mimetype);
    ...
    /**
     * VvWork.ArrayBufferReader wrapped around the audio input stream
     * provided with this VvWork.RawAudioPlayer was created.
     *
     * @private
     * @type {VvWork.ArrayBufferReader}
     */
    var reader = new VvWork.ArrayBufferReader(stream);
     ...
     * other private functions within VvWork.RawAudioPlayer, the type of the
     ...
     * VvWork.ArrayBufferReader, for example, is sufficient. Any necessary
...
VvWork.RawAudioPlayer.prototype = new VvWork.AudioPlayer();

/**
 * A description of the format of raw PCM audio received by a
 * VvWork.RawAudioPlayer. This object describes the number of bytes per
 * sample, the number of channels, and the overall sample rate.
 *
 * @private
 * @constructor
 * @param {VvWork.RawAudioPlayer._Format|Object} template
 *     The object whose properties should be copied into the corresponding
 *     properties of the new VvWork.RawAudioPlayer._Format.
 */
VvWork.RawAudioPlayer._Format = function _Format(template) {
...
/**
 * Parses the given mimetype, returning a new VvWork.RawAudioPlayer._Format
 * which describes the type of raw audio data represented by that mimetype. If
 * the mimetype is not supported by VvWork.RawAudioPlayer, null is returned.
 *
 * @private
 * @param {String} mimetype
 *     The audio mimetype to parse.
 *
 * @returns {VvWork.RawAudioPlayer._Format}
 *     A new VvWork.RawAudioPlayer._Format which describes the type of raw
 *     audio data represented by the given mimetype, or null if the given
 *     mimetype is not supported.
 */
VvWork.RawAudioPlayer._Format.parse = function parseFormat(mimetype) {
    ...
    return new VvWork.RawAudioPlayer._Format({
 ...
 * VvWork.RawAudioPlayer.
 *
 * @param {String} mimetype
 *     The mimetype to check.
 *
 * @returns {Boolean}
 *     true if the given mimetype is supported by VvWork.RawAudioPlayer,
 *     false otherwise.
 */
VvWork.RawAudioPlayer.isSupportedType = function isSupportedType(mimetype) {
...
//    if (!window.AudioContext && !window.webkitAudioContext)
    ...
    //return VvWork.RawAudioPlayer._Format.parse(mimetype) !== null;
    return VvWork.RawAudioFormat.parse(mimetype) !== null;
};

/**
 * Returns a list of all mimetypes supported by VvWork.RawAudioPlayer. Only
 ...
 *
 * @returns {String[]}
 *     A list of all mimetypes supported by VvWork.RawAudioPlayer, excluding
 *     any parameters. If the necessary JavaScript APIs for playing raw audio
 *     are absent, this list will be empty.
 */
VvWork.RawAudioPlayer.getSupportedTypes = function getSupportedTypes() {
