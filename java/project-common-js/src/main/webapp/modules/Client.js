...
/**
 * VvWork protocol client. Given a {@link VvWork.Tunnel},
 * automatically handles incoming and outgoing VvWork instructions via the
 * provided tunnel, updating its display using one or more canvas elements.
 * 
 * @constructor
 * @param {VvWork.Tunnel} tunnel The tunnel to use to send and receive
 *                                  VvWork instructions.
 */
VvWork.Client = function(tunnel, managerClient) {
    ...
    /**
     * Translation from VvWork protocol line caps to Layer line caps.
     * @private
     */
    var lineCap = {
        0: "butt",
    ...
    /**
     * The underlying VvWork display.
     *
     * @private
     * @type {VvWork.Display}
     */
    var display = !!!!managerClient ? managerClient.client.getDisplay(): new VvWork.Display(managerClient);
    
    if(!!!!managerClient){
    	Log.i("client instance display view:" + managerClient.lastScale);
    }
    /**
     * All available layers and buffers
     *
     * @private
     * @type {Object.<Number, (VvWork.Display.VisibleLayer|VvWork.Layer)>}
     */
    var layers = {};
     ...
     *
     * @private
     * @type {Object.<Number, VvWork.AudioPlayer>}
     */
    var audioPlayers = {};
     ...
     *
     * @private
     * @type {Object.<Number, VvWork.VideoPlayer>}
     */
    var videoPlayers = {};
    ...
    var stream_indices = new VvWork.IntegerPool();
    ...
    function isConnected() {
    	var isConnectedState = (currentState == STATE_CONNECTED
        || currentState == STATE_WAITING);
    	Log.e("isConnected(currentState="+currentState+"):" + isConnectedState);
    	return isConnectedState;
    }

    /**
     * Returns the underlying display of this VvWork.Client. The display
     * contains an Element which can be added to the DOM, causing the
     * display to become visible.
     * 
     * @return {VvWork.Display} The underlying display of this
     *                             VvWork.Client.
     */
     ...
     * state.
     * 
     * @param {VvWork.Mouse.State} mouseState The state of the mouse to send
     *                                           in the mouse event.
     */
    ...
    this.setClipboard = function(data) {
        ...
        var writer = new VvWork.StringWriter(stream);
     ...
     * VvWork.OutputStream using that index, associating the resulting
     * stream with this VvWork.Client. Note that this stream will not yet
     * exist as far as the other end of the VvWork connection is concerned.
     * Streams exist within the VvWork protocol only when referenced by an
     * instruction which creates the stream, such as a "clipboard", "file", or
     * "pipe" instruction.
     *
     * @returns {VvWork.OutputStream}
     *     A new VvWork.OutputStream with a newly-allocated index and
     *     associated with this VvWork.Client.
     */
    this.createOutputStream = function createOutputStream() {
        ...
        // Return new stream
        var stream = output_streams[index] = new VvWork.OutputStream(guac_client, index);
        return stream;
     ...
     * @return {VvWork.OutputStream} The created file stream.
     */
    ...
    this.createObjectOutputStream = function createObjectOutputStream(index, mimetype, name) {
        ...
        var stream = output_streams[streamIndex] = new VvWork.OutputStream(guac_client, streamIndex);
    ...
    /**
     * Fired whenever the state of this VvWork.Client changes.
     * 
     * @event
     ...
     *
     * @return {VvWork.AudioPlayer}
     *     An object which implements the VvWork.AudioPlayer interface and
     *     has been initialied to play the data in the provided stream, or null
     *     if the built-in audio players of the VvWork client should be
     *     used.
     */
    this.onaudio = null;
     ...
     *     played. It is the responsibility of the VvWork.VideoPlayer
     ...
     *     An object which implements the VvWork.VideoPlayer interface and
     *     has been initialied to play the data in the provided stream, or null
     *     if the built-in video players of the VvWork client should be
     *     used.
     */
    this.onvideo = null;
     ...
     * 
     * @event
     * @param {VvWork.InputStream} stream The stream that will receive
     ...
     * 
     * @event
     * @param {VvWork.InputStream} stream The stream that will receive data
     *                                       from the server.
     ...
     *
     * @return {VvWork.Display.VisibleLayer|VvWork.Layer}
    ...
    function getParser(index) {
        ...
        if (parser == null) {
            parser = parsers[index] = new VvWork.Parser();
    ...
    /**
     * Handlers for all instruction opcodes receivable by a VvWork protocol
     * client.
     * @private
     */
    var instructionHandlers = {
        ...
        "ack": function(parameters) {
            ...
            if (stream) {
                ...
                if (stream.onack)
                    stream.onack(new VvWork.Status(code, reason));
        ...
        "audio": function(parameters) {
            ...
            var stream = streams[stream_index] =
                    new VvWork.InputStream(guac_client, stream_index);
            ...
            if (!audioPlayer)
                audioPlayer = VvWork.AudioPlayer.getInstance(stream, mimetype);
        ...
        "body" : function handleBody(parameters) {
            ...
            if (object && object.onbody) {
                var stream = streams[streamIndex] = new VvWork.InputStream(guac_client, streamIndex);
        ...
        "clipboard": function(parameters) {
            ...
            if (guac_client.onclipboard) {
                var stream = streams[stream_index] = new VvWork.InputStream(guac_client, stream_index);
        ...
        "error": function(parameters) {
            ...
            if (guac_client.onerror)
                guac_client.onerror(new VvWork.Status(code, reason));
        ...
        "filesystem" : function handleFilesystem(parameters) {
            ...
            if (guac_client.onfilesystem) {
                var object = objects[objectIndex] = new VvWork.Object(guac_client, objectIndex);
        ...
        "img": function(parameters) {
            ...
            // Create stream
            var stream = streams[stream_index] = new VvWork.InputStream(guac_client, stream_index);
            var reader = new VvWork.DataURIReader(stream, mimetype);
        ...
        "desktop": function(isDesktop) {
            if (guac_client.isDesktop) guac_client.isDesktop(isDesktop);
        },

        "sso": function(isSSO) {
            if (guac_client.isSSO) guac_client.isSSO(isSSO);
        },

        "token_id": function(token_id) {
            if (guac_client.token_id) guac_client.token_id(token_id);
        },

        "opened": function(opened) {
            /*if (guac_client.opened) guac_client.opened(opened);*/
        },

        "destroystate": function(destroystate) {
            if (guac_client.destroystate) guac_client.destroystate(destroystate);
        },

        "display_scale": function(scale) {
        	Log.i("display scale(create tunnel):"+scale);
        	if (guac_client.display_scale) guac_client.display_scale(scale);
        },

        "display_width": function(width) {
        	Log.i("display_width：" + width);
            if (guac_client.display_width) guac_client.display_width(width);
        },

        "display_height": function(height) {
        	Log.i("display_height：" + height);
            if (guac_client.display_height) guac_client.display_height(height);
        },
        ...
        "sync": function(parameters) {
            ...
            Log.e("client sync:timestamp=" + timestamp);
            ...
            display.flush(function displaySyncComplete() {
                ...
                if (timestamp !== currentTimestamp) {
                	Log.e("send sync(flush):" + timestamp);
        ...
        "transfer": function(parameters) {
            ...
            /* Anything else that isn't a NO-OP */
                ...
                display.transfer(srcL, srcX, srcY, srcWidth, srcHeight, 
                    dstL, dstX, dstY, VvWork.Client.DefaultTransferFunction[function_index]);
        ...
        "video": function(parameters) {
            ...
            // If unsuccessful, try to use a default implementation
            if (!videoPlayer)
                videoPlayer = VvWork.VideoPlayer.getInstance(stream, layer, mimetype);
    ...
    this.disconnect = function() {
        ...
        if (currentState != STATE_DISCONNECTED
            ...
            Log.i("client send disconnect message and disconnect.");
    ...
    /**
     * Connects the underlying tunnel of this VvWork.Client, passing the
     * given arbitrary data to the tunnel during the connection process.
     *
     * @param data Arbitrary connection data to be sent to the underlying
     *             tunnel during the connection process.
     * @throws {VvWork.Status} If an error occurs during connection.
     */
    this.connect = function(data) {
        ...
        // Ping every 5 seconds (ensure connection alive)5000
...
/**
 * Map of all VvWork binary raster operations to transfer functions.
 * @private
 */
VvWork.Client.DefaultTransferFunction = {
