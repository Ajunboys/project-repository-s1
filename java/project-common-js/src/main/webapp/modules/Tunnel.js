...
/**
 * Core object providing abstract communication for VvWork. This object
 * is a null implementation whose functions do nothing. VvWork applications
 * should use {@link VvWork.HTTPTunnel} instead, or implement their own tunnel based
 * on this one.
 * 
 * @constructor
 * @see VvWork.HTTPTunnel
 */
VvWork.Tunnel = function() {
     ...
     * @type {Number}
     */
    this.state = VvWork.Tunnel.State.CONNECTING;

    /**
     * The maximum amount of time to wait for data to be received, in
     * milliseconds. If data is not received within this amount of time,
     * the tunnel is closed with an error. The default value is 2(15000).
     * 
     * @type {Number}
     */
     ...
     * 
     * @event
     * @param {VvWork.Status} status A status object which describes the
     *                                  error.
     */
    this.onerror = null;
...
/**
 * The VvWork protocol instruction opcode reserved for arbitrary internal
 ...
 * @type {String}
 */
VvWork.Tunnel.INTERNAL_DATA_OPCODE = '';
/**
...
/**
 * VvWork Tunnel implemented over HTTP via XMLHttpRequest.
 * 
 * @constructor
 * @augments VvWork.Tunnel
 *
 ...
 */
VvWork.HTTPTunnel = function(tunnelURL, crossDomain) {
	
	/*Log.i("instance of new http tunnel.");*/
    ...
    function reset_timeout() {
        ...
        receive_timeout = window.setTimeout(function () {
        	Log.i("receive_timeout close tunnel.code:" + VvWork.Status.Code.UPSTREAM_TIMEOUT);
            close_tunnel(new VvWork.Status(VvWork.Status.Code.UPSTREAM_TIMEOUT, "Server timeout."));
     ...
     * 
     * @private
     * @param {VvWork.Status} status The status causing the connection to
     *                                  close;
     */
    function close_tunnel(status) {
        ...
        if (tunnel.state === VvWork.Tunnel.State.CLOSED)
            return;

        // If connection closed abnormally, signal error.
        if (status.code !== VvWork.Status.Code.SUCCESS && tunnel.onerror) {

            // Ignore RESOURCE_NOT_FOUND if we've already connected, as that
            // only signals end-of-stream for the HTTP tunnel.
            if (tunnel.state === VvWork.Tunnel.State.CONNECTING
                    || status.code !== VvWork.Status.Code.RESOURCE_NOT_FOUND)
                tunnel.onerror(status);

        }

        // Mark as closed
        tunnel.state = VvWork.Tunnel.State.CLOSED;
        ...
        Log.i("http tunnel close:"+tunnel.state);
        

        if(!!!!status) {
        	tools.connection.httpTunnelError(status);
            // Get rid of old timeout (if any)
            window.clearTimeout(receive_timeout);
    ...
    this.sendMessage = function() {
        ...
        if (tunnel.state !== VvWork.Tunnel.State.OPEN && 
        		tunnel.state !== VvWork.Tunnel.State.CONNECTED)
            return;
         ...
         * element in a VvWork instruction.
         * 
         * @private
    ...
    function sendPendingMessages() {
        ...
        if (outputMessageBuffer.length > 0) {
        	...
        	if(outputMessageBuffer.indexOf("14.system-destroy") == 0){
        		
        		tunnel.oninstruction("destroystate", "true");
            ...
            Log.e("http-tunnel send message:" + outputMessageBuffer);
    ...
    function handleHTTPTunnelError(xmlhttprequest) {
        ...
        var code = parseInt(xmlhttprequest.getResponseHeader("VvWork-Status-Code"));
        var message = xmlhttprequest.getResponseHeader("VvWork-Error-Message");

        close_tunnel(new VvWork.Status(code, message));
        ...
        function parseResponse() {
            ...
            if (xmlhttprequest.readyState === 3 ||
                ...
                try { 
                	...
                	Log.e("recive(http tunnel):" + current);
                }
                    ...
                    if (elementEnd >= startIndex) {
                        ...
                        if (terminator === ";") {
                            ...
                            if (tunnel.oninstruction) {
                            	...
                            	Log.e("system destroy:" + opcode);
                            	
                            	if(opcode == "system-destroy"){
                                    ...
                                    var opcodes = {error:true, disconnect:true};
    ...
    this.connect = function(data) {
        ...
        connect_xmlhttprequest.onreadystatechange = function() {
            ...
            var rspData = connect_xmlhttprequest.responseText;
            var txts = rspData.split("=");
            
            tunnel_uuid = txts[0];
            
            tunnel.oninstruction("token_id", tunnel_uuid);
            
            if(txts.length == 2) {
            	
            	var otherInfos = txts[1].split(";");
            	...
            	for(var i = 0; i < otherInfos.length; i++) {
            		...
            		var instractKeyValues = otherInfos[i].split(",");
            		var instractValues = instractKeyValues[1].split(".");
            		var instractValue = instractValues[1];
            		
            		if(otherInfo.indexOf("5.desktop") == 0)
            			tunnel.oninstruction("desktop", instractValue);
            		
            		else if(otherInfo.indexOf("3.sso") == 0)
            			tunnel.oninstruction("sso", instractValue);
            		
            		else if(otherInfo.indexOf("6.opened") == 0)
            			tunnel.oninstruction("opened", instractValue);
            		
            		else if(otherInfo.indexOf("13.display_scale") == 0) {
            			var display_scale = instractKeyValues[1];
            			var display_scale_value = display_scale.substring(display_scale.indexOf(".")+1, display_scale.length);
            			tunnel.oninstruction("display_scale", display_scale_value);
            		}
            			
            		
            		else if(otherInfo.indexOf("13.display_width") == 0)
            			tunnel.oninstruction("display_width", instractValue);
            		
            		else if(otherInfo.indexOf("14.display_height") == 0)
            			tunnel.oninstruction("display_height", instractValue);
            	}
            	
            	
            }
            
            tunnel.state = VvWork.Tunnel.State.OPEN;
            if (tunnel.onstatechange)
                tunnel.onstatechange(tunnel.state);
        ...
        Log.i("http tunnel connect. ");
    };

    this.disconnect = function() {
    	Log.i("http tunnel diconnect. Manually closed.");
        close_tunnel(new VvWork.Status(VvWork.Status.Code.SUCCESS, "Manually closed."));
    };

};

VvWork.HTTPTunnel.prototype = new VvWork.Tunnel();

/**
 * VvWork Tunnel implemented over WebSocket via XMLHttpRequest.
 * 
 * @constructor
 * @augments VvWork.Tunnel
 * @param {String} tunnelURL The URL of the WebSocket tunneling service.
 */
VvWork.WebSocketTunnel = function(tunnelURL) {
    ...
    function reset_timeout() {
        ...
        // Set new timeout
        receive_timeout = window.setTimeout(function () {
        	Log.i("reset_timeout(ws) close tunnel. code:" + VvWork.Status.Code.UPSTREAM_TIMEOUT);
            close_tunnel(new VvWork.Status(VvWork.Status.Code.UPSTREAM_TIMEOUT, "Server timeout."));
        }, tunnel.receiveTimeout);
    ...
    function close_tunnel(status) {
        ...
        // If connection closed abnormally, signal error.
        if (status.code !== VvWork.Status.Code.SUCCESS && tunnel.onerror){
        	Log.i("connection closed abnormally, signal error.");
        	 tunnel.onerror(status);
        }
        ...
        tunnel.state = VvWork.Tunnel.State.CLOSED;
        if (tunnel.onstatechange){
        	Log.i("change state:" + tunnel.state);
    ...
    this.sendMessage = function(elements) {
        ...
        // Do not attempt to send messages if not connected
        if (tunnel.state !== VvWork.Tunnel.State.CONNECTED)
            return;
        ...
        if(message != "4.sync"){
            ...
            Log.e("ws-client send message:" + message);
            
            socket.send(message);
        } else {
        	Log.e("ws-client send ping time message:" + message);
        }
    ...
    this.connect = function(data) {
        ...
        // Connect socket
        socket = new WebSocket(tunnelURL + "?" + data, "vvwork");
        ...
        socket.onopen = function(event) {
            ...
            Log.i("WebSocket Open:" +tunnel.state +"->"+ tunnelURL + "?" + data);
        ...
        socket.onclose = function(event) {
        	...
        	close_tunnel(new VvWork.Status(parseInt(event.reason), event.reason));
            ...
            if(tunnel.state == 2 || tunnel.state == "2") {
            	if(!!!!tools) {
            		var parameter = {clientState : "WEB_MODEL_CRD_CONNECTED_MACHINE_ERROR"};
                	tools.notificationMessage(parameter);
            	}
            }
            
            Log.i("WebSocket Close:" + tunnel.state);
            Log.i("WebSocket Close:" + JSON.stringify(event));
        };
        
        socket.onerror = function(event) {
        	Log.i("WebSocket on error.");
            close_tunnel(new VvWork.Status(VvWork.Status.Code.SERVER_ERROR, event.data));
            Log.i("WebSocket Error:" + VvWork.Status.Code.SERVER_ERROR);
            Log.i("WebSocket Error:" + JSON.stringify(event));
        };
        ...
        socket.onmessage = function(event) {
            ...
            do {
                ...
                // If no period, incomplete instruction.
                	...
                	Log.i("recive null text.");
                	close_tunnel(new VvWork.Status(VvWork.Status.Code.SERVER_ERROR, "Incomplete instruction."));

                }

            	Log.e("recive:" + message);
                ...
                if (terminator === ";") {
                    ...
                    if (tunnel.oninstruction) {
                    	...
                    	Log.e("opcode:" + opcode+"," + elements + ":" + elements.length);
                		
                        if(opcode == "desktop")
                			tunnel.oninstruction("desktop", "true");
                		else if(opcode == "sso")
                			tunnel.oninstruction("sso", "true");
                		else if(opcode == "opened")
                			tunnel.oninstruction("opened", "true");
                		else if(opcode == "display_scale")
                			tunnel.oninstruction("display_scale", elements[0]);
                		else if(opcode == "display_width")
                			tunnel.oninstruction("display_width", elements[0]);
                		else if(opcode == "display_height")
                			tunnel.oninstruction("display_height", elements[0]);
                		else if(opcode == "token_id")
                			tunnel.oninstruction("token_id", elements[0]);
                		else  if(opcode == "system-destroy") {
    ...
    this.disconnect = function() {
    	Log.i("client ws disconnect.");
        close_tunnel(new VvWork.Status(VvWork.Status.Code.SUCCESS, "Manually closed."));
    };

};

VvWork.WebSocketTunnel.prototype = new VvWork.Tunnel();

/**
 * VvWork Tunnel which cycles between all specified tunnels until
 ...
 */
VvWork.ChainedTunnel = function(tunnelChain) {
     ...
     * 
     * @private
     * @param {VvWork.Tunnel} tunnel The tunnel to set as the current tunnel.
     */
    function attach(tunnel) {
        ...
        var failTunnel = function failTunnel(status) {
            ...
            if (status && status.code === VvWork.Status.Code.UPSTREAM_TIMEOUT) {
                tunnels = [];
                return null;
            }
        ...
        tunnel.onstatechange = function(state) {
            ...
            switch (state) {
                ...
                case VvWork.Tunnel.State.CLOSED:
    ...
    this.connect = function(data) {
        ...
        // If there IS no first tunnel, error
            ...
            chained_tunnel.onerror(VvWork.Status.Code.SERVER_ERROR, "No tunnels to try.");

    };
    
};

VvWork.ChainedTunnel.prototype = new VvWork.Tunnel();
